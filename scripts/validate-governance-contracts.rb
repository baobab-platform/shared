...
baobab-platform = entities.find { |entity| entity.fetch("id") == "BAOBAB-PLATFORM" }
fail_contract("BAOBAB-PLATFORM registry entry is required") unless baobab-platform
fail_contract("BAOBAB-PLATFORM must declare approved tenant intent") unless baobab-platform["baobab_tenant"] == true
baobab-platform_erp = baobab-platform.fetch("baobab_products").find { |entry| entry["product"] == "baobab-erp" }
fail_contract("BAOBAB-PLATFORM must declare confirmed baobab-erp consumption") unless baobab-platform_erp&.fetch("confirmed") == true
