...
- **CorporateGroup derivation is observable.** `corporate_group_derivation_total{status}` counts derivable groups by derivation status. The statuses are CURRENT, PENDING (a derivation is owed) and RETRYING (the last attempt failed, so membership has drifted from the graph until one succeeds). Membership is derived state, and a failed derivation confers and removes no access (§29).
