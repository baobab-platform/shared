...
**Governing ADRs:** ADR-SHARED-011 (this repository); ADR-SUB-0001 (`baobab-platform/baobab-subscriptions`); ADR-BCP-005, ADR-BCP-017 and ADR-BCP-018 gate ORG-11 (`baobab-platform/baobab-cp`)
...
| File | Contents |
|---|---|
| `domain.schema.json` | Identifiers (`bsub_`, `usage_`), billing state, readiness vocabulary, provider kinds, usage metric keys |
...
| `events.schema.json` and `asyncapi.yaml` | `billing-subscription.created`, `.suspended` and `.cancelled`; `usage.recorded` |
...
- **Readiness is honest.**
  - INTERNAL is `READY` with no payment dependency.
  - Money-requiring billing on a simulated provider is never `ACTIVE` and never `READY`. It is `BLOCKED` with precise reasons.
  - Suspension and cancellation show up in readiness.
